...
このアプリは GitHub Gist を使ってデータ共有できます。  
同じ `PAT` と `Gist ID` を各端末へ設定すると、同じデータを利用できます。

1. GitHubで `gist` 権限つきPATを作成
2. アプリの `クラウド同期` セクションにPATを入力
3. `新規Gist作成` を押す（初回のみ）
4. 2台目以降は同じPAT + 生成されたGist IDを入力して `設定を保存`
5. `クラウドから取得` で読み込み、編集後は `クラウドへ保存`
...
- PATは端末の`localStorage`に保存されます。共有端末では利用しないでください。
